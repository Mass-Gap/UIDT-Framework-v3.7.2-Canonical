...
Saves full tau sweep curve for documentation.
"""
import os
import json
import numpy as np
...
def run_calibration():
    ...
    # Build full tau sweep curve
    tau_curve = {}
    for tau_key in sorted(tau_scores.keys()):
        tau_curve[str(tau_key)] = {
            "score": tau_scores[tau_key],
            "fraction": round(tau_scores[tau_key] / total_configs, 4)
        }

    # Per-class rates at frozen tau
    per_class_rates = {}
    idx = 0
    for p in P_B0:
        base_n = sum(p)
        ...
        for offset in N_OFFSETS:
            ...
            for alpha in ALPHAS:
                rate = tau_results[best_tau][idx]
                class_rates.append({
                    "N": N, "alpha": alpha, "rate": round(rate, 4)
                })
                idx += 1
        per_class_rates[str(p)] = class_rates
    ...
    with open(os.path.join(out_dir, "tau.json"), "w") as f:
        json.dump({
            ...
            "total": total_configs,
            "version": "PR-B0.2",
            "tau_sweep_curve": tau_curve,
            "per_class_rates_at_frozen_tau": per_class_rates
        }, f, indent=2)