# Historical Heuristics — UIDT Museum of Superseded Methods [none]
...
A residual < 10⁻¹⁴, or a value reproduced to 80 digits, shows algorithmic convergence and
...
**Why overcome:**
- It was the *single hit* of a combinatorial scan over Casimir-like expressions (PR #367) — [none]
  selecting a near-miss is not a derivation.
- (2N_c+1)²/N_c does not appear in standard SU(N) invariant theory; the quadratic Casimirs are [none]
  C₂(F) = 4/3 and C₂(A) = 3.
- Every forward check failed: 1-loop Δγ overshoots by 34–540×; 2-loop needs d₂ = −538.8 with
  "no Casimir origin"; γ_ledger = 16339/1000 and 16339 is prime, so the "49/3 + 17/3000" split [E]
...
## 2. Δγ residual — 17 divided by 3000

**Claimed:** the small gap γ_ledger − 49/3 equals 17/3000.
**Why overcome:** the candidate formula gives 0.005652… vs reference value 0.005666… (0.247% off);
...
**Claimed:** a ~10¹⁰ suppression bridging the 10¹²⁰ cosmological-constant problem. [E]
**Why overcome:** the actual ratio λ_UIDT/r_conf ≈ 3.35×10⁶ ≈ 10^6.5, not 10¹⁰; the repo itself [none]
...
**Why overcome:** the flow seeded K_S from the squared Delta-star/gamma ratio, with a kill-switch on
...
**Reference:** PR #533; research/L4-gamma/. [none]
...
Each entry is reproduced by checking the cited PR or document reference against
the current ledger status. This file itself introduces no executable numerical
rerun.
