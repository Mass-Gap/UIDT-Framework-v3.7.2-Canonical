...
%===================================================================
% UIDT IMMUTABLE PARAMETER LEDGER MACROS (v3.9, Evidence A / A- / C)
...
%===================================================================
\newcommand{\DeltaGap}{1.710 \pm 0.015\GeV}        % [A-] local UIDT spectral-gap mapping
...
\newcommand{\LambdaGrid}{0.66\,\mathrm{nm}}         % [C]
...
dynamics~\catmark{A-} (locally established within the UIDT axiomatic framework; see
...
aims to reproduce general relativity, the Standard Model mass spectrum, and late-time
acceleration with a highly constrained parameter economy.

The local philosophical corpus also records research-mode comparisons with
external emergent-spacetime programmes, including Nathen C.~G.~Miranda's
reported work on emergent spacetime, matter, and gauge fields. In the absence
of an independently verified DOI, journal record, or arXiv identifier in the
current manuscript audit, this comparison is treated only as contextual
\catmark{E} material: it may guide future mapping between discrete causal
structures and the continuous information manifold $\Sx$, but it is not used
as evidence for any UIDT claim.
...
scalar self-coupling. In this notation the ledger value $\kappa=\kappaVal$
denotes the dimensionless EFT-normalised coefficient $\bar{\kappa}$; the
dimensionful coupling entering the dimension-5 operator is
$\bar{\kappa}/\Lambda_{\UIDT}$.
\end{axiom}
...
\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{manuscript/figures/fig_eft_operator_dimensions.pdf}
\caption{Power-counting status of the operator content entering
Axiom~\ref{ax:gauge}. Operators of dimension $d\le 4$ are renormalisable or
relevant in four spacetime dimensions, whereas the non-minimal interaction
$(\bar{\kappa}/\Lambda_{\UIDT})S F_{\mu\nu}^aF^{a\mu\nu}$ has dimension
$d=5$ and is therefore EFT-only below the cutoff $\Lambda_{\UIDT}$. This
figure encodes dimensional bookkeeping only and introduces no new physical
claim.}
\label{fig:eft-operator-dimensions}
\end{figure}
...
the scalar mass $m_S$, and the vacuum energy density) are determined by $v$ and
the ledger coefficients $\bar{\kappa}$, $\lambda_S$, and $\gamma$, with the
dimensionful EFT coupling $\bar{\kappa}/\Lambda_{\UIDT}$. This is
...
$\lambda_S$, where $\lambda_S \equiv 5\kappa^2/3=5/12\approx0.4167$ is the
exact RG relation written with its decimal display value, and hence
...
$\Delta^* = \DeltaGap$ \catmark{A-}, conditional on (i)~local existence established within the
...
\end{evidencebox}
\begin{remark}[Research-mode holographic/Shannon hypothesis]
\label{rem:gamma-shannon}
The local corpus also records an exploratory hypothesis in which
$\gamma=\GammaGeom$ functions as a holographic packing factor, relating
proton-internal Shannon information density to a Bekenstein--Hawking-like
horizon projection. This is not part of the evidential basis of the present
manuscript. Until a non-perturbative derivation connects the hypothesis to the
kinetic VEV matching with reproducible residual control, it remains a
quarantined research-mode interpretation \catmark{E} under Limitation~L4.
\end{remark}
\vspace{0.6em}
\begin{openquestion}
...
$\gamma$-scaling, the vacuum-energy ansatz contains a suppression factor
$\gamma^{12} \approx 3.62 \times 10^{14}$ for $\gamma=\GammaGeom$:
\begin{equation}
...
if only the quoted DESI uncertainty is used, or \textbf{6.91$\sigma$} when
the UIDT ledger uncertainty $0.16\kms$ is combined in quadrature. The
$w_0=-0.99$ calibration remains in \textbf{2.76$\sigma$ tension} with DESI
DR2, while $S_8 = 0.814$ remains consistent ($0.00\sigma$). Canonical values
are retained per Decision D-002; formal recalibration requires PI
authorisation. This is a
...
\begin{figure}[H]
\centering
\includegraphics[width=0.96\textwidth]{manuscript/figures/fig_desi_uidt_tension.pdf}
\caption{Cosmological calibration stress-test against DESI~DR2. The \UIDT\
ledger values are retained as internal calibrated parameters \catmark{C}, not
as replacements for external cosmological fits. The horizontal offsets show
the active tension that motivates formal recalibration after DESI Year~5 or an
equivalent precision survey.}
\label{fig:desi-uidt-tension}
\end{figure}
...
constitute a first-principles solution. The compact factor is retained as a
calibrated phenomenological ansatz, not as a first-principles 117-order
solution:
\begin{equation}
  \rho_{\UIDT}=\rho_{\mathrm{QFT}}\mathcal{C}_{99},\qquad
  \mathcal{C}_{99}\equiv\gamma^{-12}\pi^{-2}N^{-2}
  \simeq2.86\times10^{-20}.
\end{equation}
The mapping from this compact suppression factor to the observed vacuum energy
remains calibrated \catmark{C} and is covered by the open $N=99$ and residual
vacuum-energy limitations (L5 and L3). The remaining factor 2.3 is modelled as
a holographic coupling ratio \catmark{B}. The holographic factor $\pi^{-2}$ arises from the
...
encodes a specific cosmogonic narrative motivated by UIDT field dynamics.
The early universe in \UIDT\ is not a spacetime singularity
...
\begin{figure}[H]
\centering
\includegraphics[width=0.98\textwidth]{manuscript/figures/fig_falsification_exposure.pdf}
\caption{Falsification exposure of the \UIDT\ ontology manuscript. The diagram
separates theory-level, pillar-level, and prediction-level kill-switches. It is
an audit graphic: it does not promote any claim beyond its evidence class, but
records the empirical conditions under which the corresponding \UIDT\ mapping
must be revised or abandoned.}
\label{fig:falsification-exposure}
\end{figure}
...
residual), the missing-link hypothesis is refuted. In the formal lattice
limit, the torsion self-energy must obey the exact implication
\[
  E_T \to 0 \quad \Longrightarrow \quad \Sigma_T = 0 .
\]
Any implementation or phenomenological model with $E_T=0$ but
$\Sigma_T\ne0$ fails the torsion consistency condition. \emph{Current status:}
pending.
\end{killswitch}
\vspace{0.5em}
\begin{killswitch}
...
of the Standard Model. \UIDT\ models background independence not by
...
\begin{enumerate}
  \item \textbf{De Sitter Phase.} None of the examined frameworks derives the
    quasi-de Sitter inflationary phase from the internal dynamics of
    their fundamental field without introducing a separate inflaton. \UIDT\
    parameterises this via the 99-step RG cascade of $S(x)$ (Section~\ref{subsec:cosmogony}),
...
that aims to reproduce general relativity, the Standard Model mass spectrum, and
...
\UIDT\ operates with thirteen formally recognised limitations. They do not
...
\begin{enumerate}[label=\textbf{L\arabic*:}, leftmargin=*]
  ...
  \item \textbf{Vacuum Energy Factor 2.3.}
    While the $N=99$ step RG cascade provides a calibrated phenomenological
    suppression ansatz for the cosmological constant discrepancy, a residual
    factor of $\approx 2.3$ remains. It is currently
  ...
  \item \textbf{Topological Susceptibility Tension (\catmark{D}/\catmark{E}).}
    The UIDT-SVZ estimate for the fourth root of the topological susceptibility,
    $\chi_{\mathrm{top}}^{1/4}=142.98\,\mathrm{MeV}$ \catmark{D}, is in active
    tension with recent continuum SU(3) Yang--Mills lattice data
    \cite{DurrFuwa2025}, which report approximately
    $\chi_{\mathrm{top}}^{1/4}=197.8\,\mathrm{MeV}$ with few-MeV combined
    uncertainty. The current ledger classifies this as UIDT-C-056 with an
    unresolved NLO-RG research programme UIDT-C-096 \catmark{E}; it does not
    modify the spectral-gap status or the $\gamma$ sector. This is a genuine
    topological-sector attack surface, not a solved discrepancy.
...
\clearpage
  \item \textbf{EFT Status and UV Completion.}
    ...
    approximation. The Lagrangian contains a dimension-5 operator
    $(\bar{\kappa}/\Lambda_{\UIDT})S F^2$
...
\begin{itemize}
  ...
  \item \textbf{Topological Susceptibility NLO Flow.} The active C-056/C-096
    programme must determine whether NLO anomalous-dimension corrections and
    RG-flow truncation effects near $\mu\sim\Delta^*$ can reconcile the
    UIDT-SVZ estimate of $\chi_{\mathrm{top}}^{1/4}$ with the continuum
    lattice baseline of D\"urr and Fuwa~\cite{DurrFuwa2025}. Until such a
    calculation exists, the topological susceptibility mapping remains
    \catmark{D}/\catmark{E}.
...
independent axiom, this research program aims to formulate the Born rule as a
conjectural consequence of the \UIDT\ axioms and the dynamics of the vacuum
information density $S(x)$ on the Rietz Grid with lattice spacing
$\lambda = 0.66\,\mathrm{nm}$.

\begin{proposition}[Born Rule from UIDT, conjectural working formulation]
...
\]
Then the conjectural proposition states that the probability distribution of
...
Proposition~\ref{thm:born-rule-uidt} is a \emph{working formulation} of a
conjecture, not a proven result. Its resolution would promote the quantum
...
\begin{enumerate}
  ...
  \[
    ...
    = \frac{\bar{\kappa}}{\Lambda_{\UIDT}}\Sx\,
      \mathrm{Tr}(F_{\mu\nu}F^{\mu\nu})
  \]
  is conjectured to induce an effective information-norm structure
  $\int |S(x)|^2 \mathrm{d}^4x$, which plays the role of a probability norm
  in the effective description. A proof would have to show how this mirrors
  the unitary norm conservation of standard quantum mechanics.
...
To elevate Proposition~\ref{thm:born-rule-uidt} from conjecture to proven result,
...
\begin{itemize}
  ...
  decoherence (Zeh, Zurek), but constructed within the \UIDT\ axiom system
  from the coupling $\kappa$ and $E_T$.
...
Proposition~\ref{thm:born-rule-uidt} to a category \catmark{A} or \catmark{A-}
...
framework and establish the Born rule as a derived result of vacuum geometry
...
\midrule
$\lambda_S$ & Scalar self-coupling & $5\kappa^2/3=5/12\approx0.4167$ & \catmark{A} &
III & Exact RG relation; decimal is display only \\
\midrule
...
\centering
\scriptsize
\caption{Ledger exposure items not used as premises in the ontology argument.}
\label{tab:ledger_exposure}
{\setlength{\tabcolsep}{1.6pt}%
\renewcommand{\arraystretch}{0.86}%
\begin{tabular*}{\textwidth}{@{\extracolsep{\fill}}>{\raggedright\arraybackslash}p{2.65cm}>{\raggedright\arraybackslash}p{3.05cm}>{\centering\arraybackslash}p{0.95cm}>{\raggedright\arraybackslash}p{3.25cm}>{\raggedright\arraybackslash}p{3.10cm}@{}}
\toprule
\textbf{Ledger item} & \textbf{Value / exposure} & \textbf{Tag} &
\textbf{Status} & \textbf{Falsification exposure} \\
\midrule
Topological susceptibility & $\chi_{\mathrm{top}}^{1/4}=142.98\,\mathrm{MeV}$ vs. lattice $\approx197.8\,\mathrm{MeV}$ & \catmark{D}/\catmark{E} &
UIDT-C-056 tension; C-096 NLO-RG programme open & NLO-corrected value fails to approach continuum lattice within stated tolerance \\
\midrule
Quark mass hierarchy & $M(u)\!\approx\!2.44$, $M(d)\!\approx\!4.88$, $M(s)\!\approx\!93.81\,\mathrm{MeV}$, etc. & \catmark{D} &
UIDT-C-049 prediction; $m_u$ has reported pre-QED tension & Multiplicative hierarchy fails PDG/lattice constraints beyond stated thresholds \\
\midrule
Neutrino mass sum & $\Sigma m_\nu \le 0.16\,\mathrm{eV}$ from $v/\gamma^7\approx0.15\,\mathrm{eV}$ & \catmark{D} &
UIDT-C-047 cosmological prediction, not a ledger calibration & Cosmology-independent measurement gives $\Sigma m_\nu>0.25\,\mathrm{eV}$ or exactly zero \\
\midrule
Heavy exotic hadrons & $M(\Omega_{bbb})=14.4585\pm0.07\GeV$; $M(T_{cccc})=4.4982\pm0.02\GeV$ & \catmark{D}/\catmark{E} &
UIDT-C-053; tetraquark value remains high-risk analytic projection & LHCb/lattice excludes $\Omega_{bbb}$ window or finds $T_{cccc}>5.0\GeV$ \\
\midrule
Causal-graph spectral proxy & $g_{fc}=0.341\pm0.99$ & \catmark{C} &
UIDT-C-100 marginal finite-size signal at $N\sim216$ & Mean proxy fails to stay positive in large-$N$ V9-enabled runs \\
\bottomrule
\end{tabular*}
}
\end{table}
...
\ref{tab:symbol_evidence_ledger}, and~\ref{tab:ledger_exposure} are benchmarked
against the cited lattice literature and remain distinct from direct
particle-observation claims. The
...
\bibitem{DurrFuwa2025}
S.~D\"urr and G.~Fuwa,
``The topological susceptibility and excess kurtosis in SU(3) Yang--Mills
theory,''
arXiv: \href{https://arxiv.org/abs/2501.08217}{2501.08217} (2025).
