...
Q_plus = w[cut + 1:]
print("\nMultiplicative gaps (diff of logs):")
print(np.array2string(diffs, precision=2, suppress_small=True))
print("Cut index:", cut)
print("Q_plus (retained spectrum):")
...
for q in Q_plus:
    ...
    tol_val = 0.20 * max((n_star**2 - 1)/4.0, 0.75)
    print(f"q={q:.2f} -> jj={jj:.3f} -> n*={n_star}, diff={diff_val:.3f}, tol={tol_val:.3f}")
    if diff_val <= tol_val:
