...
% This manuscript retains the visual design language of v3.9.9 FINAL but
...
\begin{description}[leftmargin=1.4em,style=nextline]
\item[String-net condensation and topological order.] The most promising first literature path. String-net models construct emergent gauge fields and even emergent fermions from the collective behaviour of local degrees of freedom, and they offer the clearest existing template for how gauge structure can be genuinely \emph{emergent} rather than postulated. They are a vector for Arm~1, where the algebra must arise downstream. They are not, as they stand, a derivation of $G_{\mathrm{SM}}$ from $S(x)$.\catmark{D} \item[Quantum-link models and D-theory.] The best available benchmark for a \emph{minimal discrete gauge algebra}~\cite{brower1999,brower2004,beard1998}: these formulations realise gauge theories with finite-dimensional link variables and provide a concrete setting in which to ask what the smallest algebraic structure supporting a given gauge group is. The claim that quantum-link models constitute a proof of the passage $S(x)\to\mathrm{SU}(3)$ is rejected; their value is as a benchmark for $\mathcal{A}$, not as a closure.\catmark{D} \item[Noncommutative geometry.] The strategic fork toward an enriched primitive (Arm~2). Noncommutative geometry naturally accommodates the Standard-Model gauge group and its representations through the spectral-triple construction, and is therefore the natural home for an algebraically enriched $S$. Two overclaims are rejected: that noncommutative geometry ``eliminates'' the $d^2=0$ obstruction, and that noncommutative geometry constitutes a closed \UIDT\ solution. It is a strategic direction, carrying its own group-selection burden.\catmark{D} \item[Tensor networks.] Diagnostic tools, not origin mechanisms. Tensor-network representations are powerful for characterising entanglement structure and for probing whether a given state carries topological order, and they are valuable as instruments. They do not, by themselves, generate a gauge group, and the claim that tensor networks ``prove the gap in full'' is rejected as a closure overclaim.\catmark{D} \item[Division-algebra constructions.] A peer-reviewed programme builds Standard-Model representation content from the normed division algebras $\mathbb{R},\mathbb{C},\mathbb{H},\mathbb{O}$ and their Clifford algebras~\cite{furey2018,furey2025,gourlay2024}, and is the strongest existing source of a \emph{principled} reason for the specific factors of $\mathbb{C}\oplus\mathbb{H}\oplus M_3(\mathbb{C})$: one generation of Weyl representations arises within $\mathbb{R}\otimes\mathbb{C}\otimes\mathbb{H}\otimes\mathbb{O}$, with $\mathrm{SU}(3)$ appearing as the subgroup of $G_2=\mathrm{Aut}(\mathbb{O})$ stabilising a chosen imaginary unit. Three bounds, stated by the programme's own authors, keep it at predictive status. First, the gauge group is obtained by \emph{restricting} to symmetries preserving an inserted substructure (for example a quaternionic structure inside a $\mathbb{C}\ell(8)$ ideal breaks $\mathrm{SU}(4)$ to $\mathrm{SU}(3)\times\mathrm{U}(1)$): the structure is inserted, not generated. Second, an algebraic foundation for three generations remains open, and anomaly cancellation is flagged as future work. Third, the starting datum is a rich algebra, not a scalar; the construction presupposes the multiplicity that \cref{rem:multiplicity-precision} identifies as the unavoidable insertion. The programme is therefore an enriched-primitive (Arm~2) candidate for $\mathcal{A}$ that \emph{illustrates} the lower bounds of \cref{cor:algebra-lower-bounds} by exhibiting the inserted structure they predict as necessary; it is not a derivation of $G_{\mathrm{SM}}$ from $S(x)$ and does not rescue scalar primitivity.\catmark{D}
\end{description}
...
\begin{remark}[A moduli-space reformulation relocates the insertion; it
does not close it]
\label{rem:moduli-relocation}
One might hope to close the multiplicity gap by recasting the
(G1)--(G4) profile as a sub-variety in the moduli space of admissible
finite real spectral triples. This does not close the gap; it relocates
the insertion to the discrete index data that selects \emph{which} moduli
space exists. In the moduli formalism the space of Dirac operators is
defined only \emph{after} fixing the algebra (with its Wedderburn
decomposition), the bimodule (with its multiplicity matrix), and the
structure datum including the KO-dimension $n \bmod 8$; only the Dirac
operator then varies continuously within that fixed
space~\cite{cacic2011}. Hence the block dimensions, the real forms
$\mathbb{R}/\mathbb{C}/\mathbb{H}$, and the KO-dimension are theorem-fixed
\emph{constraints} (input labels indexing the space), whereas only the
Dirac and Yukawa data are free \emph{coordinates} within it. A sub-variety
cut inside a fixed moduli space can constrain only the latter; it is
structurally incapable of generating the former. The content of
(G1)--(G3) sits in the choice of ambient moduli space, upstream of any
sub-variety condition, which is precisely where the insertion
reappears.\catmark{A}
\end{remark}
...
\begin{remark}[Multiplicity is a condition on the fibre, not on block repetition]
\label{rem:multiplicity-precision}
The multiplicity condition (G1) is a statement about the \emph{non-abelian
fibre}: at least one Wedderburn summand must have matrix dimension
$n_i\ge 2$, so that $\mathrm{Inn}(\mathcal{A})$ is non-abelian. It is
\emph{not} the requirement that each block dimension occur more than once,
nor a constraint on the real structure $J$ forcing paired blocks. For a
finite real spectral triple the real structure requires only that, for
each bimodule summand $\mathbb{C}^{n_i}\otimes(\mathbb{C}^{n_j})^{\circ}$,
its conjugate $\mathbb{C}^{n_j}\otimes(\mathbb{C}^{n_i})^{\circ}$ be
present, a condition on the bimodule multiplicity matrix $V_{ij}$; an
anti-unitary result of Wigner guarantees that an admissible $J$ exists
already at $\dim V_{ij}=1$~\cite{vansuijlekom2019}. The Standard-Model
algebra $\mathbb{C}\oplus\mathbb{H}\oplus M_3(\mathbb{C})$, in which each
summand appears exactly once, carries a real structure of
KO-dimension~6~\cite{ccm2007}. Consequently the Standard-Model-like
partition \emph{satisfies} (G1): any claim that it violates a
``multiplicity at least two'' condition because each block appears once is
a conflation of the bimodule multiplicity matrix with block repetition and
is rejected.\catmark{A}
\end{remark}
...
\paragraph{Per-partition admissibility: verdict and mechanism must be
distinguished.} Two cautions follow from a primary-source audit of
candidate partitions, both illustrating that a correct verdict can rest on
an incorrect stated mechanism. First, the Standard-Model-like partition
$\mathbb{C}\oplus\mathbb{H}\oplus M_3(\mathbb{C})$ is anomaly-admissible:
the unimodularity reading of anomaly freedom fixes the single anomaly-free
hypercharge assignment, yielding the Standard-Model
representation~\cite{agbm1995}. This is a property of the chosen
representation and hypercharge, \emph{not} of the block partition alone;
anomaly freedom is therefore not a partition-intrinsic test.\catmark{B}
Second, among rejected test points the partition $(3,6)$ fails for a
...
$(2,2,2)$, by contrast, is not excluded by anomaly freedom at all; the
established obstruction to such symmetric configurations in the
irreducible classification is the \emph{degeneracy of the fermion mass
spectrum}~\cite{iss2004}, a dynamical-non-degeneracy criterion external to
(G1)--(G4). Its admissibility under (G1)--(G4) proper therefore remains
open.\catmark{D}

\paragraph{Methodological Boundary: Elimination at $N=6$.}
The thermodynamic kernel with block condensation, $S=\sum n_i^2$, and the off-diagonal penalty is conceptually very strong and compatible with both the IKKT/BFSS intuition and the existing \UIDT\ baseline. The sensitive points, however, are precisely those that must be explicitly marked here as assumptions: Filter~1 (the intersection constraint $|n_i - n_j| \le 1$) is not yet a theorem in this generality; Filter~2 (mass non-degeneracy) currently functions more as a dynamical plausibility argument than a proven theorem; and the global uniqueness of $(3,2,1)$, as well as the assertion that larger $N$ inevitably fragment into $(3,2,1)$ subgraphs, extends beyond what is presently mathematically secured.\catmark{E}

In the following, the result is therefore formulated as an elimination argument under explicit hypotheses, rather than as an unconditional theorem.\catmark{D}

\begin{proposition}[Conditional Selection at $N=6$]
Under the explicit hypotheses of Intersection Admissibility (Filter~1: $|n_i - n_j| \le 1$) and Mass Non-Degeneracy (Filter~2: strictly distinct non-trivial block sizes), $(3,2,1)$ is the unique thermodynamically and topologically admissible partition of $N=6$.
\end{proposition}
...
\begin{tabular}{llc}
\toprule
\textbf{Partition of $N=6$} & \textbf{Obstruction} & \textbf{Status} \\
\midrule
$(6)$ & Requires 1 block, fails Filter~2 (trivial) & Excluded \\
$(5,1)$ & $|5-1|=4 > 1$, fails Filter~1 & Excluded \\
$(4,2)$ & $|4-2|=2 > 1$, fails Filter~1 & Excluded \\
$(4,1,1)$ & $|4-1|=3 > 1$, fails Filter~1 & Excluded \\
$(3,3)$ & Degenerate mass spectrum, fails Filter~2 & Excluded \\
$(3,2,1)$ & \textbf{Unique Admissible Configuration} & \textbf{Admissible} \\
$(3,1,1,1)$ & Degenerate blocks, fails Filter~2 & Excluded \\
$(2,2,2)$ & Degenerate blocks, fails Filter~2 & Excluded \\
$(2,2,1,1)$ & Degenerate blocks, fails Filter~2 & Excluded \\
$(2,1,1,1,1)$ & Degenerate blocks, fails Filter~2 & Excluded \\
$(1,1,1,1,1,1)$ & Degenerate blocks, fails Filter~2 & Excluded \\
\bottomrule
\end{tabular}
\caption{Elimination table for $N=6$ partitions under Hypotheses H1 (Filter 1) and H2 (Filter 2).\catmark{D}}
\label{tab:n6_elimination}
\end{table}
...
W.~D.~van Suijlekom, ``A survey of spectral models of gravity coupled to matter,'' arXiv:1904.12392 (2019); in \emph{Advances in Noncommutative Geometry}, Springer (2019).
\bibitem{cacic2011}
B.~\'{C}a\'{c}i\'{c}, ``Moduli spaces of Dirac operators for finite spectral triples,'' in \emph{Quantum Groups and Noncommutative Spaces}, Aspects of Mathematics \textbf{41}, Vieweg+Teubner (2011), DOI 10.1007/978-3-8348-9831-9\_2; arXiv:0902.2068.
\bibitem{iss2004}
