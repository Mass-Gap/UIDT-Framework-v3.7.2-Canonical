...
- P1a: the exact required correction Delta_gamma_required = 17/3000.
...
def main() -> None:
    ...
    assert residual_to_gamma > mp.mpf("1e-14")
    ...
    assert gamma_bare_su4 == mp.mpf(81) / mp.mpf(4)
    ...
    assert n_su4_constant_b0_11 == mp.mpf(176)
    assert n_su4_pure_ym_b0 == mp.mpf(704) / mp.mpf(3)
    assert n_su4_difference > 0
    assert n_su4_relative_difference == mp.mpf(1) / mp.mpf(3)
