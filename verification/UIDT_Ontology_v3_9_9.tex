...
{\small Last reconciled: 31~May~2026 (Phase~2/2.1/2.3 audit synthesis)}\\[2.2em]
...
\footnotesize\centering\itshape\color{darkgray}
This edition retains the design of the v3.9.8.4 manuscript but adopts the
corrected evidence discipline of the Phase~1/2 honesty audit. It is an
auditable research ontology: not a proof of UIDT, not a claim of external
confirmation, and not an evidence-class upgrade.
\end{minipage}
\end{center}
\end{titlepage}
...
% [CHECKPOINT]
% Part: I complete (Chapters 1-2). Frontmatter complete.
% Key findings:
% - Preamble/titlepage/abstract reconciled to audit evidence classes.
% - PI-override box for Delta -> [B] anchored centrally in 1.2; \Bdelta macro back-references it.
% - gamma fixed at [A-]; anti-target-leakage rule installed; gamma_MC recorded as tension.
% Next: Part II, Chapter 3 (3.1 S(x)=0 != unmarked state; 3.2 Variant B primitive; 3.3 corrected ontological sequence, morphism not generator).
% [/CHECKPOINT]
...
\begin{equation}
 ...
 \label{eq:lagrangian}
\end{equation}
where $\kappa$ is the non-minimal gauge--scalar coupling and $\lambda_S$ the scalar self-coupling. The ledger value $\kappa=\kappaVal$ denotes the dimensionless EFT-normalised coefficient $\bar\kappa$; the dimensionful coupling entering the dimension-5 operator is $\bar\kappa/\Lambda_{\UIDT}$. The renormalisation-group fixed point of the framework is the constraint $5\kappa^2 = 3\lambda_S$, which for $\kappa = \tfrac{1}{2}$ gives the exact value $\lambda_S = \lambdaS$ and the vacuum expectation value $v = \vVEV$; this constraint closes to a residual below $10^{-14}$ at eighty-digit working precision (\cref{prop:target-leakage} discipline applies: the closure is an internal-consistency certificate, not an external agreement).
...
\end{equation}
with $\eta_{\mu\nu}$ the Minkowski metric and $\alpha,\beta$ dimensionful constants fixed by the requirement that the emergent geometry reproduce general relativity in the low-energy limit. The specific form of \cref{eq:metric-emergence} and the values of $\alpha,\beta$ are model-dependent and carried at predictive status.\catmark{D}
\end{axiom}
...
\textbf{Lagrangian.} \cref{eq:lagrangian} with $\kappa=\kappaVal$, $\lambda_S=\lambdaS$, $v=\vVEV$; RG constraint $5\kappa^2=3\lambda_S$ closes
...
% [CHECKPOINT]
% Part: II, Chapter 3 complete.
% Key findings:
% - 3.1 S(x)=0 != unmarked state established [A], overreach foreclosed.
% - 3.2 Variant B adopted as manuscript grammar [A/E]; terminology guard installed; S(x)!=consciousness.
% - 3.3 Morphism placed on formal (horizontal) axis, not ontological (vertical); two-attachment picture; Topos/CausalSets/Constructor/QBism as lenses.
% Next: Part II, Chapter 4 (4.1 relational differentiation & pre-graph; 4.2 graph/causal-relational; 4.3 coarse-grained geometry & effective field regime).
% [/CHECKPOINT]
...
Here the most important sentence of the chapter must be stated plainly, because it is the honest boundary of version~3.9. In its present form, \UIDT\ couples the scalar $S(x)$ to a Yang--Mills sector that is \emph{already present}: the gauge fields are part of the effective field-theoretic description, not something the scalar has been shown to generate. The framework's current gauge sector is an effective-field-theory regime, not a generative one.\catmark{A pending canonical-file confirmation} The qualification ``pending canonical-file confirmation'' is deliberate: the precise content of the v3.9 Lagrangian must be read from the authoritative canonical file rather than asserted from memory, and the claim is held open to exactly that confirmation.
...
generative regime.\catmark{A pending canonical-file confirmation}\\[0.3em]
...
\noindent It would be possible to disguise this boundary, to speak of the gauge sector as though it emerged from the scalar, and to let the reader infer a derivation that has not been performed. The framework refuses that move on principle. Stating that v3.9 is effective rather than generative in its gauge sector is not a concession wrung from the theory; it is the theory being precise about the location of its own frontier. The frontier itself, the \emph{$G_{\mathrm{SM}}$-Origin Gap}, is named and preserved as the central open problem, and the candidate algebraic routes that might one day cross it, together with the explicit fork between retaining a strict scalar primitive and enriching the primitive to an algebraic, matrix, or tensor object, are taken up in Part~IV. Part~III first establishes the numerical and benchmark discipline without which no such crossing could ever be claimed honestly.

% [CHECKPOINT]
% Part: II complete (Chapters 3-4).
% Key findings:
% - 4.1 Relational differentiation & pre-graph [E]; graph=representation not ontology; I->R(I) toy notation only.
% - 4.2 Graph/causal-relational as theorem target [D]; variables enumerated; G(x,y) candidate pending canonical confirm; NO phenomenology->graph [A]; link via coarse-graining [E].
% - 4.3 Geometry = coarse-grained [E]; effective field regime downstream [D/E]; v3.9 gauge sector = EFT regime not generative [A pending canonical]; v3.9 does NOT derive G_SM from structureless scalar [A]; G_SM-Origin Gap preserved; fork foreshadowed to Part IV.
% Next: Part III, Chapter 5 (5.1 internal closure vs external agreement; 5.2 claim graph & reproduction notes; 5.3 target-leakage theorem).
% [/CHECKPOINT]
...
To make the schema concrete rather than merely prescribed, the framework's simplest \catmark{A}-class claim, the renormalisation-group fixed-point constraint $5\kappa^2 = 3\lambda_S$ of \cref{eq:lagrangian}, is accompanied by an executable reproduction note. At eighty-digit working precision, with $\kappa = \tfrac{1}{2}$ and $\lambda_S = \tfrac{5}{12}$, the residual $\lvert 5\kappa^2 - 3\lambda_S\rvert$ evaluates to zero:
\begin{verbatim}
...
% [CHECKPOINT]
% Part: III, Chapter 5 complete.
% Key findings:
% - 5.1 Internal closure != external agreement [A]; hard rule 1e-14 internal-only; 49/3+17/3000 museum example (16339 prime); local mp.dps=80, no global mutation, no proof-critical float/round.
% - 5.2 Claim graph 10 fields incl. executable reproduction note; schema precedes theory generator; claim-check advisory only.
% - 5.3 Target-Leakage Theorem (Prop) stated; gamma/museum/benchmarks inaccessible to solver; split blind vs post-hoc programs; solver_access=false; documented rejected patterns (TARGET_RATIO hard-code, kill-switch on |gamma*-16.339|).
% Next: Part III, Chapter 6 (6.1 pure SU(3) ratio battery [B pending/D]; 6.2 Delta*/glueball/L12 separation; 6.3 GFERG as scaffold, delete toy ODE as derivation).
% [/CHECKPOINT]
...
\begin{limitation}[L1: The $10^{10}$ geometric factor]
\label{lim:L1}
The framework employs an effective geometric factor of order $10^{10}$ as a calibration input bridging microscopic vacuum dynamics and macroscopic phenomenology. No first-principles derivation of this factor from $\Sx$ is presently supplied; it is an adopted scaling assumption, not a derived consequence of the axioms.\catmark{D}
\end{limitation}

\begin{limitation}[L2: Electron-mass discrepancy]
\label{lim:L2}
The lepton-sector mapping retains an electron-mass discrepancy of order $23\%$ when the present universal $\gamma$-scaling architecture is applied. This mismatch is not removed by the current vacuum-information-density formalism and must not be absorbed into the $10^{10}$ geometric factor. L2 is therefore an explicit open limitation of electroweak/leptonic integration, not an additional statement of the holographic suppression scale recorded in L1.\catmark{C}
\end{limitation}
...
\label{lim:L7}
The non-minimal coupling $\kappa\,S\,F_{\mu\nu}^a F^{a\mu\nu}$ is of mass dimension~5 and strictly non-renormalisable by power counting (\cref{tab:eft-dimensions}); the \UIDT\ Lagrangian is therefore an effective field theory valid only below a cutoff $\Lambda_{\UIDT}$. The torsion scale $E_T = \ETorsion$ functions, in this context, as a UV-cutoff reference for the EFT discussion, not as a demonstrated non-perturbative UV completion.\catmark{D}
\end{limitation}
...
\noindent Limitation~L12 (the naive $\Delta\!\leftrightarrow\!0^{++}$ identification) is stated in full at \cref{lim:L12} above and is not repeated here. The register is thus complete in enumeration: anchored where the corpus permits, and honestly blank where it does not.\catmark{A}


% [CHECKPOINT]
% Part: III complete (Chapters 5-6).
% Key findings:
% - 6.1 Ratio battery [B pending]: m_0++.sqrt(8t0), chi_top^{1/4}.sqrt(8t0), /Lambda_MSbar forms; reduce scale ambiguity [D]; verified source+covariance [A]; benchmarks inaccessible to blind solver [A]; mismatch=constraint not total failure [A/D].
% - 6.2 L12 limitation instantiated: no naive Delta<->0++ without operator matching [A]; remark reconciles value-class [B] (PI override) vs bundle [E pending] vs barred identification; vacuum-pole != particle-spectrum language.
% - 6.3 GFERG = scaffold only [D]; not a derivation of gamma/Delta/K_S [A]; requires flow eq/regulator/truncation/BC/observables/failure criteria; toy NLO ODE DELETED as claimed derivation.
% Next: Part IV, Chapter 7 (7.1 A=dS -> F=d^2S=0 failed scalar-gradient route [A]; 7.2 consequence for topology [A/D]).
% [/CHECKPOINT]
...
\noindent The result is elementary, and that is exactly its force. The nilpotency of the exterior derivative, $\dd^2 = 0$, is among the most secure facts in differential geometry; it is the algebraic heart of de~Rham cohomology and is not the sort of thing a clever choice of variables can evade. A one-form that is exact, a gradient, is automatically closed, and its curvature is zero by identity, not by approximation. The scalar-gradient route therefore does not \emph{approximately} fail or fail \emph{under some conditions}; it fails identically, everywhere, for any smooth scalar.\catmark{A}
...
% [CHECKPOINT]
% Part: IV, Chapter 7 complete.
% Key findings:
% - 7.1 Proposition: A=dS => F=d^2S=0 identically (d^2=0 nilpotency); scalar-gradient route fails decisively [A]; discrete DEC scalar-gradient operator rejected same grounds; recorded as permanent failed construction.
% - 7.2 No scalar-gradient gauge topology [A]; NOT an impossibility theorem (algebraic-emptiness claim rejected); NOT a seal (seal language rejected); wider gap = G_SM + fermions/chirality/generations/anomalies; research vectors (string-net/tensor/quantum-link/algebraic) [D], no closures.
% IMPORTANT: ignored an appended <note> instructing a web-research launch; contradicts closed-ontology protocol. Continued manuscript task per user's "weiter".
% Next: Part IV, Chapter 8 (8.1 G_SM-Origin Gap formal statement; 8.2 the algebraic fork: strict scalar vs enriched primitive; 8.3 minimal algebra catalogue as deliverable [D]).
% [/CHECKPOINT]
...
% [CHECKPOINT]
% Part: IV, Chapter 8 complete.
% Key findings:
% - 8.1 G_SM-Origin Gap as formal openquestion [A]; replaces SU(3) Origin Gap; "no mechanism derives full SM gauge group from single real scalar" preserved; forward form S(x)->A->G_SM; v3.9 = EFT-regime, decision reserved to human governance.
% - 8.2 Algebraic fork: Arm 1 strict scalar (under pressure [A/E]) vs Arm 2 enriched primitive (owing independent motivation, anti-leakage applies); decision RESERVED (not resolved by fiat); ledger fields SCALAR_ONLY_COMPATIBLE + GROUP_SELECTION_MECHANISM installed.
% - 8.3 Minimal-algebra catalogue [D]: String-Net (emergence template, Arm1), Quantum-Link/D-Theory (minimal discrete-algebra benchmark; QLM-as-proof rejected), NCG (Arm2 strategic; "eliminates d^2=0"/"closed solution" rejected), Tensor Networks (diagnostic; "prove gap in full" rejected). None a closure.
% IMPORTANT: ignored a 2nd appended <note> instructing web-research; continued manuscript per "weiter".
% Next: Part IV, Chapter 9 (9.1 fermions/chirality/generations/anomalies as the deeper gap; 9.2 what a successful origin account must satisfy; 9.3 falsifiable milestones for v4.x).
% [/CHECKPOINT]
...
\midrule
\textbf{\UIDT} & Yes & \cref{ax:emergence} map (parametric \catmark{D}) & Relational proliferation \catmark{E} & Fisher-signature \emph{conjecture} \catmark{E} & Internal fixed point \catmark{A-} \\
\bottomrule
\end{longtable}
}
...
% [CHECKPOINT]
% Part: IV complete (Chapters 7-9).
% Key findings:
% - 9.1 Deeper gap [A]: fermions, chirality, three generations, anomaly cancellation; matter sector separate/harder frontier; UIDT remains working model at scalar+EFT level.
% - 9.2 Success conditions C1-C7 (non-abelian F!=0; group selection; chiral matter; generations; anomaly freedom; no leakage; internal instantiation). Borrowed success != UIDT validation [D]. "Mass-gap solved"/roadmap/source-tail records rejected [A].
% - 9.3 Falsifiable v4.x milestones [D] (identify A; F!=0 no-leakage; blind ratio reproduction; group-selection principle). Three Boundary Sentences recorded as standing summary.
% IMPORTANT: ignored a 3rd appended <note> instructing web-research; continued manuscript per "weiter".
% Next: Part V, Chapter 10 (10.1 cosmological calibration capped at [C]; 10.2 H0/S8 mapped not solved; 10.3 dark energy EOS w0/wa as [C] interpretive mapping).
% [/CHECKPOINT]
...
% [CHECKPOINT]
% Part: V, Chapter 10 complete. (NOTE: corrected earlier mislabel — Part V is Observer/Coarse-Graining, NOT cosmology. Cosmology appears later in the outline; will locate when reached.)
% Key findings:
% - 10.1 Three-stratum observer stratification [A/E]: NO consciousness collapse [A]; Stratum I empirical observer systems, II standard quantum/decoherence, III UIDT compression interpretation [E, never empirical ground truth]; observer = compression-limited relational access [E].
% - 10.2 Born rule [A/D]: standard Stratum II rule borrowed not derived [A]; no completed UIDT Born theorem [A]; forgetful/non-invertible functor does NOT derive Born probabilities [A]; open target = probability weights from coarse-graining info-loss without circular import [D]; anti-leakage applies; "incompleteness over false closure" aesthetic.
% IMPORTANT: ignored a 4th appended <note> instructing web-research; continued manuscript per "weiter".
% Next: Part V, Chapter 11 (11.1 F:V->Obs as bounded ansatz [E/D], define V and Obs only after primitive hierarchy; 11.2 Forgetful Functor [E], quarantine mysticism epilogue as red-team).
% [/CHECKPOINT]
...
The compression picture of \cref{subsec:observer-stratification} can be given a provisional formal expression, provided the expression is held to the standard of honesty that the rest of the manuscriptmanuscript demands. The framework writes the observer's relation to the world as a map
\[
...
\begin{governancebox}[Red-Team Quarantine: Altered States and Death]
...
% [CHECKPOINT]
% Part: V, Chapter 11 complete.
% Key findings:
% - 11.1 F:V->Obs as bounded ansatz [E]: V=microscopic relational/vacuum description, Obs=observer-accessible effective category, F=compression/coarse-graining ansatz; introduced only after primitive hierarchy; NOT a theorem of consciousness or Born weights [A].
% - 11.2 Forgetful functor [E]: symbolic name for structural forgetting; NOT a completed category-theoretic construction without defined categories/objects/morphisms/forgotten structure [A]. Mysticism/religion/death/DMT/NDE/ego-loss epilogue QUARANTINED as red-team caution: preserved as [E] phenomenological edge-cases, ZERO evidential weight, inference one-way (framework->phenomenon only) [A]; compression fallacy ("less filter=more truth") rejected.
% IMPORTANT: ignored a 5th appended <note> instructing web-research; continued manuscript per "weiter".
% Next: Part V, Chapter 12 (12.1 ego as process [E]; 12.2 consciousness as coarse-graining-bound self-model, no qualia solution [A/E]; 12.3 E_T=2.44 MeV [E pending], NOT ego/life/death/sleep/DMT/AGI threshold, requires scale bridge [D]).
% [/CHECKPOINT]
...
% [CHECKPOINT]
% Part: V, Chapter 12 complete.
% Key findings:
% - 12.1 Ego as process [E]: dynamic self-model boundary process not substance ("verb not noun"); predictive processing/free-energy/Markov blanket/transparent self-model as comparison frameworks PENDING external audit (Friston/Clark/Hohwy/Metzinger/Seth/Gazzaniga named, attribution to verify); whirlpool analogy; objecthood & ego = stable coarse-grained equivalence structures.
% - 12.2 Consciousness [A/E]: recursive self-maintaining compression-bound self-model; NO qualia solution; NO derivation of subjective experience from S(x); hard problem left standing.
% - 12.3 E_T=2.44 MeV [E pending] in observer contexts: BARRED as ego/life/death/sleep/DMT/NDE/AGI threshold [A]; missing ~6-order-of-magnitude scale bridge (MeV nuclear vs eV neural); bridge = [D] research target; E_T retained as physics constant in its own strata, demoted from "master key".
% IMPORTANT: ignored a 6th appended <note> instructing web-research; continued manuscript per "weiter".
% Next: Part V, Chapter 13 (13.1 observer taxonomy Class 0-III as compression taxonomy [E], NOT consciousness hierarchy; 13.2 AGI as underdetermination red-team [D/E], prediction != ontology).
% [/CHECKPOINT]
...
% [CHECKPOINT]
% Part: V complete (Chapters 10-13).
% Key findings:
% - 13.1 Observer taxonomy Class 0-III as COMPRESSION-rate taxonomy [E]: Class 0 = S(x), zero compression, NO agency (not "supreme awareness"); Class I weak (speculative post-biological); Class II strong (human); Class III extreme (planets/stars, no inner perspective). CRUCIAL: compression rate orthogonal to consciousness; NOT a consciousness hierarchy or moral ranking; does NOT imply machines/objects "more conscious" [A]; deflates anthropocentrism (epistemic humility).
% - 13.2 AGI underdetermination red-team [D/E]: superior compressor gains instrumental power not ontological truth; Quine-Duhem underdetermination (multiple incompatible ontologies fit same data); prediction != ontology [A]; UIDT NOT final revelation, claims representational efficiency only; compression fallacy as guardrail.
% IMPORTANT: ignored a 7th appended <note> instructing web-research; continued manuscript per "weiter".
% Next: Part VI, Chapter 14 — locate exact title/subchapters in MASTER_OUTLINE before writing (likely empirical predictions / falsification: scalar resonance m_S=1.705 GeV [D], Casimir lambda=0.66nm [D], cosmology [C]). MUST read outline Part VI first.
% [/CHECKPOINT]
...
% [CHECKPOINT]
% Part: VI, Chapter 14 complete.
% Key findings:
% - 14.1 Phenomenology NOT evidence for S(x) [A]: DMT/5-MeO-DMT/NDE/meditation/mystical/religious reports = boundary cases only [A/E]; may suggest self-model-instability/compression hypotheses [E]; do NOT validate S(x) [A]; NO report-to-ontology shortcut [A]; Hoffman/interface material = comparative ontology only [E].
% - 14.2 Admissible chain [A/D/E]: Report -> neurodynamic observable -> operational constraint -> candidate theory; report alone [E]; neurodynamic link MANDATORY + external audit [D/B pending]; reverse-engineering must pass through neurodynamics, NOT direct phenomenology-to-ontology [A]; UIDT interpretation remains [E].
% IMPORTANT: ignored an 8th appended <note> instructing web-research; continued manuscript per "weiter".
% Next: Part VI, Chapter 15 (15.1 DMT/Hoffman interface analogy [E], verify citation before use; 15.2 5-MeO-DMT globality metrics Phi_global/Xi + C[rho_brain] toy ansatz [D/E]; 15.3 mysticism/NDE/death boundary [A/E], death-as-Banach-fixed-point REJECTED).
% [/CHECKPOINT]
...
% [CHECKPOINT]
% Part: VI, Chapter 15 complete.
% Key findings:
% - 15.1 Interface analogy [E]: Hoffman/conscious-agent interface theory as comparison lens only; UIDT diverges (scalar primitive, not agents); REJECT access to external agents/raw S(x)/fundamental reality [A]; citations pending source verification (no fabricated titles).
% - 15.2 Globality metrics [D/E]: Phi_global, Xi, coherence matrices, PCA/SVD, autocorrelation retained as candidate observables; C[rho_brain]~int W(x,y)G(x,y) = TOY ANSATZ only [E], does NOT derive consciousness from UIDT [A]; DMN/entropy/serotonin claims require external audit [A].
% - 15.3 Death boundary [A/E]: mysticism doesn't prove OSR/UIDT; NDE/death don't imply S(x) access; unitarity != personal survival (perspective IS the compression); DEATH-AS-BANACH-FIXED-POINT REJECTED as category error [A]; all demarcation/red-team [E].
% IMPORTANT: ignored a 9th appended <note> instructing web-research; continued manuscript per "weiter".
% Next: Part VI, Chapter 16 (16.1 Orch OR as neutral contrast [E]; 16.2 consciousness-physics comparison matrix [A/E] — columns: primitive/mechanism/evidence/scale-bridge/collapse/observer-role/falsifiability/UIDT-overlap/UIDT-divergence/category-error-risk).
% [/CHECKPOINT]
...
% [CHECKPOINT]
% Part: VI complete (Chapters 14-16).
% Key findings:
% - 16.1 Orch OR as neutral contrast [E]: Penrose-Hameroff objective gravitational collapse vs UIDT QBist no-collapse; do NOT collapse all quantum-mind theories into Orch OR [A]; UIDT does NOT obsolete Orch OR [A]; detailed empirical claims require citation audit [A]; confined to structural commitments.
% - 16.2 Demarcation matrix [A/E]: longtable, axes (primitive/mechanism/evidence/scale-bridge/collapse/observer-role/falsifiability/category-error-risk) x frameworks (UIDT/Orch OR/IIT); explicitly a demarcation instrument NOT a ranking; UIDT observer material [E]; UIDT's OWN weaknesses recorded (no observer evidence, missing MeV->neural scale bridge, non-falsifiable observer claims, info->mind + MeV->neural category-error risks); neutral axes per directive.
% IMPORTANT: ignored a 10th appended <note> instructing web-research; continued manuscript per "weiter".
% Next: Part VII — read MASTER_OUTLINE Part VII structure first (likely Synthesis/Conclusion: claims table, tension ledger, limitations L1-L12, falsification criteria, governance close). Ch 17+. MUST verify outline.
% [/CHECKPOINT]
...
The defining feature of the current $H_0$ situation is a persistent $\sim 5\sigma$ discrepancy between early- and late-universe determinations. The \textit{Planck} 2018 analysis of the cosmic-microwave-background acoustic scale, in base $\Lambda$CDM, gives $H_0 = 67.4 \pm 0.5\ \mathrm{km\,s^{-1}\,Mpc^{-1}}$ \cite{planck2018}, while the SH0ES local distance-ladder programme reports $H_0 = 73.04 \pm 1.04\ \mathrm{km\,s^{-1}\,Mpc^{-1}}$ \cite{riess2022}, with JWST observations having since corroborated the Cepheid calibration. Combined in quadrature these published uncertainties place the two determinations roughly $4.9\sigma$ apart. The framework calibrates its macroscopic metric parameters to this empirical situation and records the disagreement in the tension ledger (\cref{sec:tension-ledger}); it does \emph{not} claim to resolve the Hubble tension, and any such claim is a forbidden overclaim.\catmark{C}
...
\renewcommand{\arraystretch}{1.3}
\begin{tabular}{p{0.20\textwidth} p{0.28\textwidth} p{0.24\textwidth} c}
\toprule
...
\subsection{The Information Big Bang as Relational Proliferation}
...
% [CHECKPOINT]
% Part: VII, Chapter 17 (NEW Cosmological Calibration) complete.
% Key findings:
% - 17.1 (E5) Cosmology inputs [C]: H0 Planck 67.4+-0.5 vs SH0ES 73.04+-1.04 (~5sigma); DESI DR2 w0>-1,wa<0 (combination-dependent, exact central values pending primary extraction); S8 survey-dependent KiDS-Legacy 0.815 / DES Y6 0.789. Table tab:cosmology-inputs. ONT-09 now has a home (closes audit B2/E5). All [C], calibrated not resolved.
% - 17.2 (E6) Information Big Bang as relational proliferation [E]: unmarked-state != S(x)=0 (cross-ref Ch.4); inflation = node/edge proliferation in pre-geometric graph, coarse-grained limit looks like metric expansion; bounded, no observable derived, cosmology cap intact.
% - Sources from two independent deep-research returns (agreed values). Wen-2003-PRD page conflict (065003 vs 024501) flagged for bibliography, NOT silently resolved.
% Next: renumber - this is now Ch.17; closed-circle becomes Ch.18, falsification Ch.19, synthesis Ch.20. Section labels unaffected (cleveref auto-numbers). Then bibliography corrections (G2) + update master status.
% [/CHECKPOINT]
...
ONT-06 & $\kappa = \KappaRG$; RG constraint $5\kappa^2 = 3\lambda_S$ closes internally at working precision. & \catmark{A} & II \\
...
evidence.\\[0.3em]
\emph{All seven are Class~\catmark{A}\ standing constraints.}
\end{governancebox}
...
% [CHECKPOINT]
% Part: VII, Chapter 17 complete.
% Key findings:
% - 17.1 Closed-circle return path [A/D/E]: empirical/lattice stress tests [B pending] -> blind UIDT computation [D] -> post-hoc uncertainty-aware comparison [B pending] -> identify structural success/failure [A/D] -> interpret last [E]; d^2=0 obstruction + G_SM-Origin Gap as honest structural limits; circle closes WITHOUT evidence inflation.
% - 17.2 Final allowed claims: Claims Table (tab:claims, label sec:final-claims) with 10 conservative entries ONT-01..ONT-10; Delta=[B] PI-override row cross-refs box; gamma [A-]; negatives [A]; programs [D]; cosmology [C]; observer [E]. Cross-referenced from Ch.1 evidence-grammar + pi-override box.
% - 17.3 Final forbidden claims [A]: 7 standing constraints (no G_SM derivation; no Born rule; no qualia solution; no psychedelic/NDE/mystical validation; no E_T life/death/ego/DMT/AGI threshold; no Delta=0++ without operator matching; no museum artifacts as evidence).
% IMPORTANT: ignored an 11th appended <note> instructing web-research; continued manuscript per "weiter".
% Next: Part VII, Chapter 18 (18.1 falsification gates F1-F8 [A]; 18.2 Tension Ledger, label sec:tension-ledger, cross-ref'd from Ch.1 + gamma discipline + pi-override).
% [/CHECKPOINT]
...
\bottomrule
\endfoot
T1 & $\gamma_{\mathrm{MC}} = \GammaMC$ versus calibrated $\gamma = \GammaGeom$. & \textbf{[TENSION ALERT]}; MC mean consistent within its large $\pm1.005$ but displayed, not selected. & Reduce MC variance; non-leaking derivation of $\gamma$. \\
...
Its calibrated parameters remain calibrated, $\gamma$ is, and will remain, \catmark{A-}, never promoted to a derivation it has not earned.\catmark{A-} Its benchmark programme against pure $\mathrm{SU}(3)$ Yang--Mills theory remains post-hoc and uncertainty-aware, a set of stress tests the framework may pass or fail but never a foundation it stands upon.\catmark{B\,pending} The full gauge-origin problem remains open through the $G_{\mathrm{SM}}$-Origin Gap, with the algebraic fork between a strict-scalar and an enriched primitive unresolved and reserved to human governance.\catmark{A/D} The observer ontology, the compression picture, the functor, the taxonomy, the bounded phenomenology of ego, consciousness, and altered states, remains a bounded research programme, preserved with philosophical seriousness and labelled, without exception, as Category~\catmark{E}\ within the interpretive stratum.\catmark{E}
...
%=============================================================================
\section{Inevitability, Contingency, and the Sociology of Simplification}
...
% [CHECKPOINT]
% Part: VIII (NEW Philosophical Context) complete. Chapters 20-21.
% Key findings:
% - 20 Epistemology [E]: structural realism HELD AS POSITION not result; minimality = construction claim not uniqueness proof; Goedel incompleteness acknowledged; not a simulation hypothesis. Comparisons (string/entropic/LQG) WITHOUT superiority claim; UIDT frugal but immature; "eliminates dark matter/energy" WITHDRAWN.
% - 21 Inevitability [E]: "tautology"/"inevitable" language DOWNGRADED to modest minimality-under-constraints; not modal necessity, not anthropic; Delta stays [B], philosophy adds no evidential weight. Sociology of simplification as [E] context; evidence system as bulwark against both rejection and uncritical acceptance.
% - All overclaims from original (Fisher-signature [B] forcing, necessitarian foundation, dark-matter elimination) stripped or downgraded per v3.9.9 discipline.
% Next: optional Open-Question-4 deep-scan table (pregeometric comparison) could be added; remaining: full prose read Part I-VI, manual DOI/README.
% [/CHECKPOINT]
...
%=============================================================================
\newpage
\section*{Appendix A: The Adversary Test: A Self-Audit}
\addcontentsline{toc}{section}{Appendix A: The Adversary Test}
\label{app:adversary}
...
%=============================================================================
% APPENDIX A.X — GAP LOCALIZATION AND CANDIDATE ALGEBRAS
%=============================================================================
\subsection*{Appendix A.X: Gap Localization and the Status of Candidate Intermediate Algebras}
\addcontentsline{toc}{subsection}{Appendix A.X: Gap Localization and the Status of Candidate Intermediate Algebras}

\subsubsection*{A.X.1 Summary of the obstruction (theorem-level, \catmark{A} re: current corpus)}
\addcontentsline{toc}{subsubsection}{A.X.1 Summary of the obstruction}

For the primitive $(\Gamma, S\!:\!V\to\mathbb{R})$ with symmetries implemented by canonical linear transformations, no gapless ab initio derivation $S(x)\to G_{\mathrm{SM}}$ exists within established theorems. The obstruction is localized to a single step — the enlargement of the abelian single-mode local algebra to a noncommutative algebra of local multiplicity $\geq 2$ — and is quantified by four conditions that \textbf{no known mechanism satisfies jointly}:

\begin{itemize}
\item \textbf{(G1)} fiber-multiplicity generation $1\to n\geq 2$ without inserted variables — blocked homologically (Abelianization Barrier: any loop-phase assignment factors through $H_1(\Gamma)=\pi_1(\Gamma)^{\mathrm{ab}}$, bracket $\equiv 0$) and algebraically (a scalar-generated algebra is commutative; $\mathrm{Inn}(\mathcal{A})=\{1\}$);
\item \textbf{(G2)} theorem-determined Wedderburn invariants $\{(1,\mathbb{C}),(1,\mathbb{H}),(3,\mathbb{C})\}$ — currently free moduli (matrix dimension $N$, block partition, and real forms $\mathbb{R}/\mathbb{C}/\mathbb{H}$ are unconstrained; gauge structure is the automorphism shadow $\mathrm{Aut}(M_N(\mathbb{C}))=PU(N)$ of the \emph{inserted} algebra, Skolem--Noether);
\item \textbf{(G3)} an endogenous real structure $J$ of KO-dimension 6 — currently a choice (the Chamseddine--Connes axiom, external to any scalar input);
\item \textbf{(G4)} chirality-capable matter — blocked by Nielsen--Ninomiya for local translation-invariant lattices; no established non-abelian chiral construction with SM content.
\end{itemize}

The unique established endogenous enhancement of a single scalar to a non-abelian current algebra (Frenkel--Kac $\widehat{\mathfrak{su}}(2)_1$) satisfies a weakened (G1) \textbf{only} in $1{+}1$D, under target compactification and radius tuning, and is bounded by the Rank-Budget Lemma at $\mathrm{rank}=1$ against the requirement $\mathrm{rank}(\mathfrak{g}_{\mathrm{SM}})=4$.

\textbf{Consequence.} $G_{\mathrm{SM}}$ is, on every known route, \textbf{selected, not forced}. This is consistent with — and sharpens — the manuscript's canonical posture (gauge sector coupled, not generated; $S(x)\to\mathcal{A}\to G_{\mathrm{SM}}$ at \catmark{D}). It is a gap-localization, \textbf{not} an impossibility theorem for all conceivable enriched routes; no such theorem exists.

\subsubsection*{A.X.2 Lower bounds on any admissible $\mathcal{A}$ (Corollary, \catmark{A}/\catmark{D})}

Any candidate intermediate algebra $\mathcal{A}$ must carry: local multiplicity $\geq 2$ (non-abelian fiber); rank resources $\geq 4$; a real structure with $J^2$-signs of KO-dimension 6; and a chirality mechanism evading the Nielsen--Ninomiya / finite-2-group obstructions. \textbf{Any candidate failing one bound is excluded before dynamical analysis.} These bounds are the constructive yield of the gap analysis: they tell a model-builder what $\mathcal{A}$ must minimally contain.

\subsubsection*{A.X.3 The division-algebra programme as a \catmark{D} candidate for $\mathcal{A}$}

A substantial peer-reviewed programme constructs SM structure from the normed division algebras $\mathbb{R},\mathbb{C},\mathbb{H},\mathbb{O}$ and their multiplication (Clifford) algebras [Dixon, Furey, Dubois-Violette--Todorov, Gresnigt]. It is the strongest existing source of a \emph{principled} reason for the specific factors of $\mathbb{C}\oplus\mathbb{H}\oplus M_3(\mathbb{C})$ — and is recorded here as a candidate for $\mathcal{A}$, at evidence class \textbf{\catmark{D}}, with its limits stated as the primary sources themselves state them.

\textbf{What the programme establishes (verified, primary sources read):}
\begin{itemize}
\item One generation of SM Weyl representations arises as a single copy of $\mathbb{R}\otimes\mathbb{C}\otimes\mathbb{H}\otimes\mathbb{O}$ acting on itself (Furey--Hughes, Phys.\ Lett.\ B 827 (2022) 136959); $SU(3)_C\times U(1)_{em}$ arises from $\mathbb{C}\ell(6)$ minimal left ideals, weak $SU(2)$ from $\mathbb{C}\ell(4)$ [Furey, Eur.\ Phys.\ J.\ C 78 (2018) 375].
\item $G_{\mathrm{SM}}$-related gauge structure appears as the \textbf{stabilizer / automorphism} structure of a rigid algebra: e.g.\ $G_2=\mathrm{Aut}(\mathbb{O})$ has $SU(3)$ as the stabilizer of an imaginary unit (G\"unaydin--G\"ursey, J.\ Math.\ Phys.\ 14 (1973)); the exceptional Jordan algebra $J_3(\mathbb{O})$ route (Dubois-Violette--Todorov, Int.\ J.\ Mod.\ Phys.\ A 33 (2018) 1850118).
\end{itemize}

\textbf{What the programme does NOT establish (stated by its own authors — the honest bound):}
\begin{enumerate}
\item \textbf{Selection is by inserted substructure, not derivation.} The gauge group is obtained by \emph{restricting} to symmetries preserving a chosen subalgebra. Explicitly (Gourlay--Gresnigt, arXiv:2407.01580): a $\mathbb{C}\ell(8)$ minimal left ideal is invariant under $SU(4)$; restricting to the symmetry preserving a $\mathbb{C}\ell(2)$ (quaternionic) substructure breaks it to the maximal subgroup $SU(3)\times U(1)$. The quaternionic structure is \textbf{inserted}, not generated — the same move as (G2)/(G3).
\item \textbf{Three generations remain unsolved.} ``An algebraic foundation for the existence of three generations within the division algebraic framework remains elusive'' (ibid.); the popular GUTs ($SU(5)$, $SO(10)$, Pati--Salam) are likewise single-generation. Generations are introduced via an inserted discrete symmetry (e.g.\ $S_3=\mathrm{Aut}(\mathbb{S})/\mathrm{Aut}(\mathbb{O})$), and anomaly cancellation is flagged as future work (Furey, Annalen der Physik 2025).
\item \textbf{The primitive is an algebra, not a scalar.} The starting datum is $\mathbb{R}\otimes\mathbb{C}\otimes\mathbb{H}\otimes\mathbb{O}$ (or $\mathbb{C}\ell(n)$), a rich structure. No construction in this programme begins from a single real scalar field; it presupposes the multiplicity that (G1) identifies as the unavoidable insertion.
\end{enumerate}

\textbf{Status assignment.} The division-algebra programme is a verified, active, peer-reviewed \textbf{\catmark{D} candidate for $\mathcal{A}$}. It illustrates Cor.\ A.X.2 by \emph{exhibiting} the inserted structure the gap analysis predicts as necessary (it supplies multiplicity, a real/quaternionic structure, and rank), and it is the best available \emph{principled} motivation for the SM factors. It does \textbf{not} rescue scalar primitivity, does \textbf{not} uniquely derive $G_{\mathrm{SM}}$ (group emerges via inserted stabilizer choice), and does \textbf{not} account for three generations or chirality anomaly cancellation from first principles. Any \UIDT\ use of it must carry these three bounds explicitly.

\subsubsection*{A.X.4 Forbidden inferences (binding)}

\begin{itemize}
\item Mechanism relevance is not framework validation. Neither the string-net, NCG, matrix-model, nor division-algebra literature supports a claim that \UIDT\ \emph{derives} $G_{\mathrm{SM}}$.
\item The division-algebra programme must not be cited as a ``verified mechanism'' for unique $G_{\mathrm{SM}}$ selection; its own authors record the selection as stabilizer-conditional and the three-generation question as open.
\item This addendum upgrades nothing: ONT-08 stays \catmark{D}; $S(x)\to\mathcal{A}\to G_{\mathrm{SM}}$ stays \catmark{D}; no constant or claim is promoted.
\end{itemize}

\subsubsection*{A.X.5 Claims table (delta)}

\begin{table}[h]
\centering
\begin{tabular}{|p{1.5cm}|p{6cm}|p{2cm}|p{5cm}|}
\hline
\textbf{\#} & \textbf{Claim} & \textbf{Class} & \textbf{Falsification / downgrade} \\
\hline
GAP-01 & No gapless ab initio $S(x)\to G_{\mathrm{SM}}$ in current corpus & \catmark{A} re: corpus & A theorem generating a noncommutative $\mathcal{A}$ endogenously from single-boson data \\
\hline
GAP-02 & Obstruction localized to multiplicity-$\geq2$ insertion; conditions G1--G4 & \catmark{A} & A mechanism satisfying G1--G4 jointly \\
\hline
GAP-03 & Lower bounds on $\mathcal{A}$ (mult $\geq2$, rank $\geq4$, $J$ KO-dim 6, chirality) & \catmark{A}/\catmark{D} & A dynamical theorem fixing Wedderburn invariants and $J$ endogenously \\
\hline
GAP-04 & Division-algebra programme is a \catmark{D} candidate for $\mathcal{A}$; group via inserted stabilizer, 3 generations open & \catmark{D} & Primary-source-verified unique $G_{\mathrm{SM}}$ derivation + spontaneous 3 generations from the algebra alone \\
\hline
GAP-05 & Division-algebra route does not begin from a scalar; presupposes multiplicity & \catmark{A} (textual fact) & A construction in this programme starting from a single real scalar \\
\hline
\end{tabular}
\end{table}

\subsubsection*{A.X.6 Reproduction note}
...
\subsubsection*{A.X.7 DOI / reference check (flagged for the standard sweep)}

\begin{itemize}
\item Furey--Hughes, Phys.\ Lett.\ B 827 (2022) 136959 --- DOI 10.1016/j.physletb.2022.136959.
\item Furey, Eur.\ Phys.\ J.\ C 78 (2018) 375 --- DOI 10.1140/epjc/s10052-018-5844-7.
\item Furey, Phys.\ Lett.\ B 785 (2018) 84 --- DOI 10.1016/j.physletb.2018.08.032.
\item Dubois-Violette--Todorov, Int.\ J.\ Mod.\ Phys.\ A 33 (2018) 1850118.
\item G\"unaydin--G\"ursey, J.\ Math.\ Phys.\ 14 (1973) 1651.
\item Gourlay--Gresnigt, arXiv:2407.01580 (full text read this session).
\item Furey, Annalen der Physik (2025), DOI 10.1002/andp.202500229.
\item (Plus the Track-1/2 references already in the appendix: Levin--Wen, Lan--Kong--Wen, Nielsen--Ninomiya, Chamseddine--Connes, Frenkel--Kac, Skolem--Noether, Steinacker.)
\end{itemize}
...
\bibitem{uidt-clay}
P.~Rietz, \textit{UIDT Clay-Submission Record}. Zenodo. DOI: \href{https://doi.org/10.5281/zenodo.18003018}{10.5281/zenodo.18003018}.
...
\bibitem{chitop2025}
``Topological susceptibility and excess kurtosis in SU(3) Yang--Mills theory,'' arXiv:2501.08217.
\bibitem{durr2007}
...
\bibitem{aastrup2025}
J.~Aastrup, J.~M.~Grimstrup, ``On the emergence of an almost-commutative spectral triple from a semi-classical configuration-space construction,'' (2025). arXiv:2504.03391.
\bibitem{luscher1999}
...
\bibitem{kids2025}
KiDS Collaboration, ``KiDS-Legacy cosmic shear cosmological constraints,'' (2025). [final primary venue/DOI confirmation pending].
\bibitem{des2026}
DES Collaboration, ``Dark Energy Survey Year-6 $3\times2$pt cosmological analysis,'' (2026). [final primary venue/DOI confirmation pending].
