...
/-- [DESIGN-LEVEL] Full pipeline (trivial version).
    BlockPartition → FiniteAlgebraSignature → SpectralTriple ℂ ℂ.
...
def BlockPartition.toTrivialTriple {N : ℕ} (p : BlockPartition N)
    (ko : Fin 8 := standardModelKODim) :
    SpectralTriple ℂ ℂ :=
  FiniteAlgebraSignature.toTrivialTriple (BlockPartition.toSignature p) ko
...
example {N : ℕ} (p : BlockPartition N) : RepUnital (BlockPartition.toTrivialTriple p).rep := by
  unfold RepUnital BlockPartition.toTrivialTriple FiniteAlgebraSignature.toTrivialTriple
  ext x
  simp [AlgebraRep.trivialC]

example {N : ℕ} (p : BlockPartition N) : RepRespectsMul (BlockPartition.toTrivialTriple p).rep := by
  unfold RepRespectsMul BlockPartition.toTrivialTriple FiniteAlgebraSignature.toTrivialTriple
  intro a b
  ext x
  simp [AlgebraRep.trivialC]
  ring
...
example : (BlockPartition.toTrivialTriple p321).signature.blocks = (BlockPartition.toSignature p321).blocks := by
  simp [BlockPartition.toTrivialTriple, FiniteAlgebraSignature.toTrivialTriple, BlockPartition.toSignature]
...
example : (BlockPartition.toTrivialTriple p321).firstOrderCond = True := by
  simp [BlockPartition.toTrivialTriple, FiniteAlgebraSignature.toTrivialTriple, FirstOrderCondition]

example : (BlockPartition.toTrivialTriple p321).orientable = True := by
  simp [BlockPartition.toTrivialTriple, FiniteAlgebraSignature.toTrivialTriple, Orientable]
