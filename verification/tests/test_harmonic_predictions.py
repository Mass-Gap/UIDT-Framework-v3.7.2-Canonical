...
class TestHarmonicPredictor:
    ...
    def test_initialization(self):
        ...
        # Residual check < 1e-5
        assert abs(self.predictor.f_vac - self.vacuum_freq_gev) < 1e-5
        assert abs(self.predictor.delta - self.delta_gap_gev) < 1e-5
    ...
    def test_generate_report(self):
        ...
        assert abs(float(report["Omega_bbb_GeV"]) - float(omega_mass)) < 1e-5
        assert abs(float(report["X2370_Resonance_GeV"]) - float(self.predictor.predict_x2370_resonance())) < 1e-5
    ...
    def test_check_proton_anchor(self):
        ...
        assert abs(float(result["f_vac_MeV"]) - float(f_vac_mev)) < 1e-5
        assert abs(float(result["ratio"]) - float(ratio)) < 1e-5
        assert abs(float(result["deviation"]) - float(deviation)) < 1e-5
